<?php
// Promise Pillar Edit — Viata Luxe Guesthouse (Track B)
$db = Database::get();
$row = null;
$id = (int)($_GET['id'] ?? 0);
if ($id) {
    $stmt = $db->prepare('SELECT * FROM promise_pillars WHERE id = :id');
    $stmt->execute(['id' => $id]);
    $row = $stmt->fetch() ?: null;
}
if (!$id) $row = ['sort_order' => 0, 'is_published' => 1, 'page_id' => 1, 'icon' => '◐'];
$isNew = $row === null || !isset($_GET['id']) || (int)($_GET['id'] ?? 0) === 0;
$r = $row ?? [];
?>
<div class="admin-page">
  <div class="page-header page-header-inline">
    <div><h2><?= $isNew ? 'New Promise Pillar' : 'Edit Pillar' ?></h2><p class="muted small"><?= $isNew ? 'Add pillar' : 'Editing "' . e($r['title'] ?? '') . '"' ?></p></div>
    <a href="/admin/promise-pillars" class="btn btn-outline">&larr; Back</a>
  </div>
  <div class="form-card"><div class="form-card__body form-card__body--narrow">
    <form method="POST" action="/admin/api/crud.php" data-ajax>
      <?= csrf_field() ?>
      <input type="hidden" name="entity" value="promise_pillar">
      <input type="hidden" name="action" value="save">
      <input type="hidden" name="id" value="<?= (int)($r['id'] ?? 0) ?>">
      <input type="hidden" name="page_id" value="1">
      <div class="form-group">
        <label>Icon (e.g. ◐ ⬢ ✦ ◉ ☾)</label>
      ...
      <div class="form-group">
        <label>Text *</label>
        <textarea name="text" required><?= e($r['text'] ?? '') ?></textarea>
      </div>
      <div class="form-group">
        <label>Link URL (optional)</label>
        <input type="text" name="link_url" value="<?= e($r['link_url'] ?? '') ?>">
      </div>
      <div class="form-row">
        <div class="form-group">
      ...
      <div class="form-actions">
        ...
        <a href="/admin/promise-pillars" class="btn btn-outline">Cancel</a>
      </div>
    </form>
  </div></div>
</div>
